...
export function reportInput(raw) {
  ...
  if (b.agreed !== true) throw new Error('개인정보 처리 동의가 필요해요.');
  ...
  return {email,headline:text(b.headline,100).trim()||'상권 분석 리포트',sub:text(b.sub,200),facts:rows(b.facts,['label','value','tag']),survey:rows(b.survey,['label','value']),zones:rows(b.zones,['name','value']),honesty:'매출은 상권 집계에서 계산한 추정치예요. 입력 조건과 계산값은 사용자가 제공했으며 서버가 검증한 개별 점포 실적이 아닙니다.'};
}
...
function html(d) {
  ...
  <div style="max-width:600px;margin:0 auto;padding:28px 20px">
    ...
    <p style="font-size:11px;color:#8B95A1;text-align:center;margin:16px 0 0">
      이 리포트의 입력 조건과 계산값은 사용자가 제공한 참고 자료이고, 개별 점포의 실적이나 수익을 보장하지 않아요.
    </p>
  </div></body></html>`;
}
...
async function deliver(req,res) {
  ...
  if (process.env.REPORT_EMAIL_ENABLED !== 'true' || !process.env.BREVO_API_KEY || !process.env.REPORT_FROM_EMAIL) return res.status(503).json({error:'메일 발송은 준비 중이에요. 미리보기와 CSV 저장을 이용해 주세요.'});
  try {
    ...
    if (!response.ok) { console.error('[report] upstream status',response.status); return res.status(502).json({error:'메일 발송을 요청하지 못했어요. 잠시 후 다시 시도해 주세요.'}); }
    return res.status(200).json({ok:true});
  } catch(e) {return res.status(502).json({error:safeError('report',e,'발송 결과를 확인하지 못했어요. 수신함을 먼저 확인해 주세요')});}
}
...
export default async function handler(req,res) {
  ...
  if(!/^[a-zA-Z0-9-]{16,80}$/.test(key))return res.status(400).json({error:'요청 식별자가 필요해요. 화면을 새로고침해 주세요.'});
  ...
  if(request&&request.signature!==signature)return res.status(409).json({error:'입력 내용이 바뀌었어요. 화면을 새로고침한 뒤 다시 시도해 주세요.'});
  if(!request){
    if(requests.size>=1000)return res.status(503).json({error:'발송 요청이 많아요. 잠시 후 다시 시도해 주세요.'});
