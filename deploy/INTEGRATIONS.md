...
|R-ONE|`RONE_API_KEY`|통계마다 `RONE_STATBL_ID`, `RONE_CYCLE`, `RONE_CLS_ID`, `RONE_ITM_ID`도 필요|
...
`data.go.kr`은 하나의 통합 데이터 API가 아닙니다. 같은 서비스키를 쓰더라도 상가정보, 주차장, K-Startup 등 필요한 API를 각각 활용신청해야 합니다. R-ONE도 키만으로는 어떤 통계표의 어느 지역·항목을 가져올지 결정되지 않으므로 네 가지 조회 코드를 함께 설정해야 합니다.
