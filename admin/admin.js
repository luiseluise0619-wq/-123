...
function armLock(){clearTimeout(lockTimer);lockTimer=setTimeout(()=>{lock();el('status').textContent='15분이 지나 잠겼어요.';},15*60000);}
async function request(path,body={},options={}){
  // 같은 URL의 Nginx Basic 인증도 Authorization 헤더를 쓴다. 2차 키까지
  // 그 헤더에 넣으면 브라우저가 기억한 1차 인증과 서로 덮어쓰므로 전용 헤더로 분리한다.
  const headers={'Content-Type':'application/json'};if(token&&!options.sessionOnly)headers['X-Mysbizon-Admin-Key']=token;
  const response=await fetch('api/'+path,{method:'POST',headers,credentials:'same-origin',body:JSON.stringify(body),signal:AbortSignal.timeout(10000)});
  if(!response.ok){
    const hadToken=!!token;
    let message='요청을 처리하지 못했습니다.';
    try{const data=await response.json();if(data&&data.error)message=data.error;}catch{}
    if(response.status===401||response.status===403){lock({clearKey:false});message=hadToken?'2차 관리자 키가 맞지 않습니다.':'2차 관리자 인증이 만료되었습니다.';}
    else if(response.status===429)message='입력 횟수가 많아 잠시 잠겼습니다. 60초 뒤 다시 시도해 주세요.';
    throw new Error(message+' ('+response.status+')');
  }
  armLock();return response;
}
function paint(){
...
async function showWorkspace(){await load();el('key').value='';el('login').hidden=true;el('workspace').hidden=false;el('logout').hidden=false;}
el('login-form').onsubmit=e=>{e.preventDefault();run(async()=>{token=el('key').value.trim();if(!/^[a-f0-9]{64}$/i.test(token))throw new Error('2차 관리자 키는 공백 없는 영문·숫자 64자리입니다.');await request('session');token='';await showWorkspace();});};
el('logout').onclick=()=>run(async()=>{try{await request('logout',{}, {sessionOnly:true});}finally{lock();el('status').textContent='관리자 화면을 잠갔어요.';}});
...
window.addEventListener('pagehide',()=>{token='';clearTimeout(lockTimer);});
run(async()=>{try{await request('session',{}, {sessionOnly:true});await showWorkspace();}catch{lock();el('status').textContent='';}});
